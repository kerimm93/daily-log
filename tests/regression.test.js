...
// Card merges preserve the app's newest-first card order without changing generic entity ordering.
{
  const c = context(['getCloseDayItemFreshness', 'canonicalMergeFingerprint', 'isDeleted', 'mergeById', 'legacyPlanTaskId', 'normalizePlanTasks',
    'mergePlanTasks', 'mergePlanWakeTime', 'mergeCloseDayPlan', 'mergeArchivedDayPlan', 'mergeArchivedDay', 'mergeDays'], {
    S: { deletedIds: { deleted: '2026-01-04T00:00:00.000Z' } }
  });
  const newer = { id: '2000000000000-new', text: 'neu', createdAt: '2026-01-03T00:00:00.000Z' };
  const older = { id: '1000000000000-old', text: 'alt', createdAt: '2026-01-01T00:00:00.000Z' };
  const local = { date: '2026-01-06', cards: [newer, older], objects: [], feedItems: [], plan: {} };
  assert.deepEqual(plain(c.mergeArchivedDay({}, local).cards.map(card => card.id)), [newer.id, older.id]);

  const remoteOnly = { id: '1500000000000-remote', text: 'remote', createdAt: '2026-01-02T00:00:00.000Z' };
  const merged = c.mergeArchivedDay({ date: '2026-01-06', cards: [older], objects: [], feedItems: [], plan: {} },
    { date: '2026-01-06', cards: [newer, remoteOnly], objects: [], feedItems: [], plan: {} });
  assert.deepEqual(plain(merged.cards.map(card => card.id)), [newer.id, remoteOnly.id, older.id]);
  assert.deepEqual(plain(c.mergeArchivedDay({ date: '2026-01-06', cards: [newer, remoteOnly], objects: [], feedItems: [], plan: {} },
    { date: '2026-01-06', cards: [older], objects: [], feedItems: [], plan: {} }).cards.map(card => card.id)), [newer.id, remoteOnly.id, older.id]);
  assert.deepEqual(plain(c.mergeArchivedDay(merged, { date: '2026-01-06', cards: [older], objects: [], feedItems: [], plan: {} })), plain(merged));

  const editedOld = { ...older, text: 'alt bearbeitet', updatedAt: '2026-01-05T00:00:00.000Z' };
  assert.equal(c.mergeArchivedDay({ date: '2026-01-06', cards: [older], objects: [], feedItems: [], plan: {} },
    { date: '2026-01-06', cards: [editedOld], objects: [], feedItems: [], plan: {} }).cards.find(card => card.id === older.id).text, 'alt bearbeitet');
  const tombstoned = c.mergeArchivedDay({ date: '2026-01-06', cards: [{ id: 'deleted', createdAt: '2026-01-04T00:00:00.000Z' }], objects: [], feedItems: [], plan: {} },
    { date: '2026-01-06', cards: [newer], objects: [], feedItems: [], plan: {} });
  assert.deepEqual(plain(tombstoned.cards.map(card => card.id)), [newer.id]);
  const idless = c.mergeArchivedDay({ date: '2026-01-06', cards: [{ text: 'legacy A' }, { text: 'legacy A' }], objects: [], feedItems: [], plan: {} },
    { date: '2026-01-06', cards: [{ text: 'legacy B' }], objects: [], feedItems: [], plan: {} });
  assert.equal(idless.cards.filter(card => !card.id).length, 3);
  const otherOrder = c.mergeArchivedDay({ date: '2026-01-07', cards: [], objects: [{ id: 'b' }, { id: 'a' }], feedItems: [{ id: 'fb' }, { id: 'fa' }], plan: {} },
    { date: '2026-01-07', cards: [], objects: [], feedItems: [], plan: {} });
  assert.deepEqual(plain(otherOrder.objects.map(item => item.id)), ['a', 'b']);
  assert.deepEqual(plain(otherOrder.feedItems.map(item => item.id)), ['fa', 'fb']);
}
...
// Reclosing a past day updates, but never duplicates, migration targets.
{
  ...
  const future = c.S.futurelog[0];
  const futureId = future.id; const futureCreatedAt = future.createdAt;
  assert.equal(future.date, '2026-02-01');
  assert.equal(future.month, '2026-02');
  future.done = true;
  ...
  assert.equal(future.id, futureId); assert.equal(future.createdAt, futureCreatedAt); assert.equal(future.done, true);
  assert.equal(future.date, '2026-03-01'); assert.equal(future.month, '2026-03');
  assert.equal(future.text, 'Geplant aktuell');
  assert.equal(future.updatedAt, '2026-01-11T00:00:00.000Z');
  day.objects[1].reviewNote = 'Ohne Termin';
  ControlledDate.set('2026-01-12T00:00:00.000Z');
  c.finishDay();
  assert.equal(future.date, ''); assert.equal(future.month, '');
  assert.equal(future.id, futureId); assert.equal(future.createdAt, futureCreatedAt); assert.equal(future.done, true);
  assert.equal(future.updatedAt, '2026-01-12T00:00:00.000Z');
  ControlledDate.set('2026-01-13T00:00:00.000Z');
  c.finishDay();
  assert.equal(future.updatedAt, '2026-01-12T00:00:00.000Z');
  assert.equal(c.S.futurelog.filter(item => item.objId === 'plan' && item.fromDate === targetDate).length, 1);
...
// Global delete/consume actions tombstone merge-synchronized records.
{
  const ControlledDate = controlledDate('2026-06-01T00:00:00.000Z');
  const deletedIds = {};
  const mergeCtx = context(['getCloseDayItemFreshness', 'canonicalMergeFingerprint', 'isDeleted', 'mergeById', 'mergeDeletedIds'], {
    S: { deletedIds }
  });
  const assertNotRestored = (arr, id) => assert.equal(mergeCtx.mergeById([], arr, 'id').some(item => item.id === id), false);

  const zCtx = context(['recordDeletion', 'deleteZettel'], {
    Date: ControlledDate, S: { deletedIds, zettels: [{ id: 'zettel-del', title: 'Alt' }] },
    save() {}, renderZettelTab() {}, updateZettelBadge() {}
  });
  zCtx.deleteZettel('zettel-del');
  assert.equal(zCtx.S.deletedIds['zettel-del'], '2026-06-01T00:00:00.000Z');
  assertNotRestored([{ id: 'zettel-del', title: 'Remote' }], 'zettel-del');

  const objects = [{ id: 'obj-with-col', collectionIds: ['col-del'], updatedAt: '2026-05-01T00:00:00.000Z' }];
  const colCtx = context(['touchUpdatedAt', 'stampItemEditMeta', 'touchObj', 'recordDeletion', 'removeCollectionFromAllObjects', 'deleteCurrentCollection'], {
    Date: ControlledDate, activeKolId: 'col-del', S: { deletedIds, collections: [{ id: 'col-del', name: 'Alt' }], days: [] },
    TODAY: { objects }, confirm: () => true, getDeviceId: () => '', getDeviceName: () => '', save() {}, renderKollektionenTab() {}, toast() {}
  });
  colCtx.deleteCurrentCollection();
  assert.equal(colCtx.S.deletedIds['col-del'], '2026-06-01T00:00:00.000Z');
  assert.deepEqual(plain(objects[0].collectionIds), []);
  assert.equal(objects[0].updatedAt, '2026-06-01T00:00:00.000Z');
  assertNotRestored([{ id: 'col-del', name: 'Remote' }], 'col-del');

  const acceptedObjects = [];
  const pufferCtx = context(['touchUpdatedAt', 'stampItemEditMeta', 'recordDeletion', 'acceptPuffer', 'dismissPuffer', 'dismissAllPuffer'], {
    Date: ControlledDate, S: { deletedIds, migrationPuffer: [
      { id: 'p-accept', text: 'A', fromDate: '2026-01-01' },
      { id: 'p-dismiss', text: 'B', fromDate: '2026-01-01' },
      { id: 'p-all-a', text: 'C' }, { id: 'p-all-b', text: 'D' }, { text: 'legacy-no-id' }
    ] }, getViewData: () => ({ objects: acceptedObjects }), uid: () => 'new-obj', getDeviceId: () => '', getDeviceName: () => '',
    save() {}, renderPuffer() {}, updateBadges() {}, updateFLBadge() {}, toast() {}
  });
  pufferCtx.acceptPuffer('p-accept');
  assert.equal(pufferCtx.S.deletedIds['p-accept'], '2026-06-01T00:00:00.000Z');
  assert.equal(acceptedObjects.length, 1);
  pufferCtx.dismissPuffer('p-dismiss');
  assert.equal(pufferCtx.S.deletedIds['p-dismiss'], '2026-06-01T00:00:00.000Z');
  pufferCtx.dismissAllPuffer();
  assert.equal(pufferCtx.S.deletedIds['p-all-a'], '2026-06-01T00:00:00.000Z');
  assert.equal(pufferCtx.S.deletedIds['p-all-b'], '2026-06-01T00:00:00.000Z');
  assert.equal(Object.prototype.hasOwnProperty.call(pufferCtx.S.deletedIds, 'undefined'), false);
  assertNotRestored([{ id: 'p-accept', text: 'Remote' }, { id: 'p-dismiss', text: 'Remote' }, { id: 'p-all-a' }], 'p-accept');

  const trashTarget = { cards: [], objects: [] };
  const trashCtx = context(['touchUpdatedAt', 'stampItemEditMeta', 'recordDeletion', 'ensureTrash', 'restoreTrashItem', 'hardDeleteTrashItem', 'emptyTrash'], {
    Date: ControlledDate, S: { deletedIds, trash: { cards: [
      { id: 'trash-card-restore', data: { id: 'card-restore', text: 'Card' }, originalDate: '2026-01-01' },
      { id: 'trash-card-hard', data: { id: 'card-hard', text: 'Card' } },
      { id: 'trash-card-empty', data: { id: 'card-empty', text: 'Card' } }
    ], objects: [
      { id: 'trash-object-empty', data: { id: 'object-empty', text: 'Obj' } }
    ] } }, getDataForDate: () => trashTarget, getDeviceId: () => '', getDeviceName: () => '',
    save() {}, renderTrashModal() {}, refreshAllTabs() {}, toast() {}
  });
  trashCtx.S.deletedIds['card-restore'] = 'old';
  trashCtx.restoreTrashItem('card', 'trash-card-restore');
  assert.equal(trashCtx.S.deletedIds['card-restore'], undefined);
  assert.equal(trashCtx.S.deletedIds['trash-card-restore'], '2026-06-01T00:00:00.000Z');
  assert.equal(trashTarget.cards.length, 1);
  trashCtx.hardDeleteTrashItem('card', 'trash-card-hard');
  assert.equal(trashCtx.S.deletedIds['trash-card-hard'], '2026-06-01T00:00:00.000Z');
  trashCtx.emptyTrash();
  assert.equal(trashCtx.S.deletedIds['trash-card-empty'], '2026-06-01T00:00:00.000Z');
  assert.equal(trashCtx.S.deletedIds['trash-object-empty'], '2026-06-01T00:00:00.000Z');
  assertNotRestored([{ id: 'trash-card-hard' }, { id: 'trash-card-empty' }, { id: 'trash-object-empty' }], 'trash-card-hard');
  const mergedDeleted = mergeCtx.mergeDeletedIds({ keep: '2026-06-01T00:00:00.000Z' }, { keep: '2026-06-02T00:00:00.000Z', other: '2026-06-01T00:00:00.000Z' });
  assert.deepEqual(plain(mergedDeleted), { keep: '2026-06-02T00:00:00.000Z', other: '2026-06-01T00:00:00.000Z' });
  assert.deepEqual(plain(mergeCtx.mergeById([], [{ id: 'trash-card-hard' }], 'id')), []);
}

// Card trash restore tombstones consumed linked object trash entries by trash-entry ID.
{
  const ControlledDate = controlledDate('2026-06-10T00:00:00.000Z');
  const target = { cards: [], objects: [] };
  const c = context(['touchUpdatedAt', 'stampItemEditMeta', 'recordDeletion', 'ensureTrash', 'restoreTrashItem',
    'getCloseDayItemFreshness', 'canonicalMergeFingerprint', 'isDeleted', 'mergeById'], {
    Date: ControlledDate, getDataForDate: () => target, getDeviceId: () => '', getDeviceName: () => '',
    S: { deletedIds: {
      'card-tree': 'old', 'obj-tree-a': 'old', 'obj-tree-b': 'old',
      'card-linked': 'old', 'obj-linked-a': 'old', 'obj-linked-b': 'old'
    }, trash: { cards: [
      { id: 'trash-card-tree', originalDate: '2026-01-01', data: { id: 'card-tree', text: 'Tree card',
        _deletedObjectTree: [{ id: 'obj-tree-a', text: 'A' }, { id: 'obj-tree-b', text: 'B' }] } },
      { id: 'trash-card-linked', originalDate: '2026-01-01', data: { id: 'card-linked', text: 'Linked card',
        _deletedLinkedObjectIds: ['obj-linked-a', 'obj-linked-b'] } }
    ], objects: [
      { id: 'trash-obj-tree-a', data: { id: 'obj-tree-a', text: 'A trash' } },
      { id: 'trash-obj-tree-b', data: { id: 'obj-tree-b', text: 'B trash' } },
      { id: 'trash-obj-tree-unrelated', data: { id: 'obj-tree-x', text: 'X trash' } },
      { id: 'trash-obj-linked-a', data: { id: 'obj-linked-a', text: 'LA trash' } },
      { id: 'trash-obj-linked-b', data: { id: 'obj-linked-b', text: 'LB trash' } },
      { id: 'trash-obj-linked-unrelated', data: { id: 'obj-linked-x', text: 'LX trash' } }
    ] } }, save() {}, renderTrashModal() {}, refreshAllTabs() {}, toast() {}
  });

  c.restoreTrashItem('card', 'trash-card-tree');
  assert.equal(target.cards.some(card => card.id === 'card-tree'), true);
  assert.deepEqual(plain(target.objects.map(obj => obj.id).sort()), ['obj-tree-a', 'obj-tree-b']);
  assert.equal(c.S.deletedIds['card-tree'], undefined);
  assert.equal(c.S.deletedIds['obj-tree-a'], undefined);
  assert.equal(c.S.deletedIds['obj-tree-b'], undefined);
  assert.equal(c.S.deletedIds['trash-card-tree'], '2026-06-10T00:00:00.000Z');
  assert.equal(c.S.deletedIds['trash-obj-tree-a'], '2026-06-10T00:00:00.000Z');
  assert.equal(c.S.deletedIds['trash-obj-tree-b'], '2026-06-10T00:00:00.000Z');
  assert.equal(c.S.trash.objects.some(entry => entry.id === 'trash-obj-tree-a'), false);
  assert.equal(c.S.trash.objects.some(entry => entry.id === 'trash-obj-tree-b'), false);
  assert.equal(c.S.trash.objects.some(entry => entry.id === 'trash-obj-tree-unrelated'), true);
  assert.equal(c.S.deletedIds['trash-obj-tree-unrelated'], undefined);
  assert.equal(Object.prototype.hasOwnProperty.call(c.S.deletedIds, 'undefined'), false);
  assert.equal(Object.prototype.hasOwnProperty.call(c.S.deletedIds, ''), false);
  c.restoreTrashItem('card', 'trash-card-tree');
  assert.equal(target.cards.filter(card => card.id === 'card-tree').length, 1);
  assert.equal(target.objects.filter(obj => obj.id === 'obj-tree-a').length, 1);

  c.restoreTrashItem('card', 'trash-card-linked');
  assert.equal(target.cards.some(card => card.id === 'card-linked'), true);
  assert.equal(target.objects.some(obj => obj.id === 'obj-linked-a'), true);
  assert.equal(target.objects.some(obj => obj.id === 'obj-linked-b'), true);
  assert.equal(c.S.deletedIds['obj-linked-a'], undefined);
  assert.equal(c.S.deletedIds['obj-linked-b'], undefined);
  assert.equal(c.S.deletedIds['trash-card-linked'], '2026-06-10T00:00:00.000Z');
  assert.equal(c.S.deletedIds['trash-obj-linked-a'], '2026-06-10T00:00:00.000Z');
  assert.equal(c.S.deletedIds['trash-obj-linked-b'], '2026-06-10T00:00:00.000Z');
  assert.equal(c.S.trash.objects.some(entry => entry.id === 'trash-obj-linked-a'), false);
  assert.equal(c.S.trash.objects.some(entry => entry.id === 'trash-obj-linked-b'), false);
  assert.equal(c.S.trash.objects.some(entry => entry.id === 'trash-obj-linked-unrelated'), true);
  assert.equal(c.S.deletedIds['trash-obj-linked-unrelated'], undefined);

  const remoteTrashObjects = [
    { id: 'trash-obj-tree-a', data: { id: 'obj-tree-a' } },
    { id: 'trash-obj-linked-a', data: { id: 'obj-linked-a' } },
    { id: 'trash-obj-tree-unrelated', data: { id: 'obj-tree-x' } }
  ];
  const mergedTrash = c.mergeById(c.S.trash.objects, remoteTrashObjects, 'id');
  assert.equal(mergedTrash.some(entry => entry.id === 'trash-obj-tree-a'), false);
  assert.equal(mergedTrash.some(entry => entry.id === 'trash-obj-linked-a'), false);
  assert.equal(mergedTrash.some(entry => entry.id === 'trash-obj-tree-unrelated'), true);
  assert.equal(c.S.deletedIds['obj-tree-a'], undefined);
  assert.equal(c.S.deletedIds['obj-linked-a'], undefined);
}
