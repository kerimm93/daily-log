...
// Actual task UI actions assign IDs, update freshness and record deletions.
{
  ...
  let nextId = 0;
  const c = context(['legacyPlanTaskId', 'normalizePlanTasks', 'ensurePlanState', 'addPlanTask', 'togglePlanTask', 'removePlanTask'], {
...
{
  const c = context(['legacyPlanTaskId', 'normalizePlanTasks', 'mergePlanTasks', 'mergePlanWakeTime', 'mergeCloseDayPlan', 'mergeArchivedDayPlan', 'filterDeletedFeedItems', 'mergeById', 'mergeDays'], {
...
// The actual JSON file merge path applies symmetric plan freshness to a past TODAY.
{
  ...
  const c = context(['legacyPlanTaskId', 'normalizePlanTasks', 'mergePlanTasks', 'mergePlanWakeTime', 'mergeCloseDayPlan', 'mergeArchivedDayPlan', 'importFileAndMerge'], {
...
// Daily Note input uses only public plan fields, never merge metadata.
{
  const day = { cards: [], objects: [], feedItems: [], reviewDone: false, plan: {
    ...
    tasks: [{ id: 'technical-id', text: 'Sichtbar', updatedAt: '2026-01-02T00:00:00.000Z' }],
  ...
  assert.doesNotMatch(c.getDailyNoteFullDayInput(), /planField(?:Cleared|Edited|sExplicitlyEdited)At|planFieldsExplicitlyEdited|aufstehzeit(?:EditedAt|ExplicitlyEdited)|planTaskDeletedAt|planTasksClearedAt|technical-id|2026-01-02T00:00:00\.000Z/);
}
